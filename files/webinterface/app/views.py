...
def upgrade(request):
    ...
    if request.POST.get('check_usb') == '1':
        Popen(["/usr/sbin/upgrader check_usb"], shell=True, stdout=PIPE, close_fds=True).communicate()[0]
        step = 'format_usb'

    if request.POST.get('format_usb') == '1':
        Popen(["/usr/sbin/upgrader format_usb"], shell=True, stdout=PIPE, close_fds=True).communicate()[0]
        step = 'backup_to_usb'

    if request.POST.get('backup_to_usb') == '1':
        Popen(["/usr/sbin/upgrader backup_to_usb &> /tmp/backup_output"], shell=True, stdout=PIPE, close_fds=True)
        show_output = True
        step = 'backup_to_usb'
    ...
    if request.POST.get('download_image') == '1':
        Popen(["/usr/sbin/upgrader download_image"], shell=True, stdout=PIPE, close_fds=True).communicate()[0]
